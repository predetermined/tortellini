...
const languages: { [language: string]: Language; } = {
    javascript: {
        colors: [
            {
                regex: {
...
export class Tortellini {
    ...
    private readonly adviser: HTMLDivElement;
    private readonly initialHTML: string;
    private readonly singleSpaceCharacter: string = "\u00a0";
    private readonly tabCharacter: string;
    private readonly language: "javascript" | "html" | "css";
    private isAdviserOpen: boolean = false;
    private openedAdviserByInteraction: boolean;
    private currentTabOffset: number;

    constructor(base: HTMLElement, language: "javascript" | "html" | "css", {caretColor, indentSize}: Options = {}) {
        if (!base) throw Error("Please provide a base element as the first argument.");
        if (!language || (language !== "javascript" && language !== "html" && language !== "css")) throw Error("Please define the programming language (javascript, html or css) as the second argument.")
        ...
        const styles = `
            * {
            ...
            [data-tortellini-editor], [data-tortellini-highlights] {
                white-space: pre-wrap;
                min-height: 100%;
            }
            
            [data-tortellini-editor] {
                color: transparent;
                caret-color: ${caretColor || "#fff"};
                z-index: 10;
                position: sticky;
                outline: none;
                width: 100%;
            ...
            [data-tortellini-adviser]:empty, [data-tortellini-adviser] > a:nth-child(1n + 7) {
                display: none;
            }
            
            [data-tortellini-adviser] > a {
                width: 100%;
                padding: 5px;
                display: block;
                font-weight: normal;
            ...
            [data-tortellini-adviser] > a[data-active] {
                position: relative;
            }
            
            [data-tortellini-adviser] > a[data-active]::after {
                content: "";
                position: absolute;
                top: 0;
                left: 0;
        ...
        this.tabCharacter = this.singleSpaceCharacter.repeat(indentSize || 4);

        this.base = base;
        if (getComputedStyle(this.base).position === "static") this.base.style.position = "relative";
        ...
        this.editor.onkeyup = this.handleAdviserBehavior.bind(this);
        this.base.onkeydown = this.handleKeys.bind(this);
    }
    ...
    private getSuggestions(focusWord: string, {object = null, showOnlyGoodMatching = false}: { object?: string; showOnlyGoodMatching?: boolean; } = {}): string[] {
        ...
        if (object && window[object] && Object.getOwnPropertyNames(window[object])) {
            ...
            if (window[object].__proto__) {
                availableSuggestions.unshift(...Object.getOwnPropertyNames(window[object].__proto__));
            }
        }
        ...
        const declaredVariables: string[] = languages[this.language].variableDeclarers
            ?.reduce((declaredVariables: string[], declarer: string): string[] => {
                const foundDeclaredVariables: string[] = this.editor.innerText
                    .match(new RegExp(`${declarer} ([a-zA-Z0-9]*)`, "g"))
                    ?.map(declaredVariable => declaredVariable.replace(declarer, "").trim());

                if (foundDeclaredVariables) {
                    return [
                        ...declaredVariables,
                        ...foundDeclaredVariables
                    ];
                }

                return declaredVariables;
            }, [])
            .filter((declaredVariable: string): boolean => declaredVariable.startsWith(focusWord) && declaredVariable !== focusWord);
    ...
    private insertSuggestion(selectedElement: HTMLElement) {
    ...
    private showSuggestions({askedForSuggestions = true}: { askedForSuggestions?: boolean; } = {}) {
        this.isAdviserOpen = false;
    ...
    private colorCode() {
        ...
        languages[this.language].colors.map((colorInformation: ColorOption) => {
    ...
    private handleAdviserBehavior(e: KeyboardEvent) {
    ...
    private handleEditorSpecificKeys(e: KeyboardEvent) {
        switch (e.key) {
            case "{": {
                ...
                (document.getSelection() as ModernSelection).modify("move", "backward", "character");
                break;
            }
            case "[": {
                const selection: ModernSelection = window.getSelection() as ModernSelection;
                const range: Range = selection.getRangeAt(0);
            ...
            case ")": {
                ...
                e.preventDefault();
                (document.getSelection() as ModernSelection).modify("move", "forward", "character");
                break;
            }
            case "`": {
                document.execCommand("insertHTML", false, "`");
    ...
    private handleKeys(e: KeyboardEvent) {
        ...
        if ((!suggestionKeyPressed && (e.target as HTMLElement).dataset.adviser) || (e.target as HTMLElement).hasAttribute("data-tortellini-editor"))
